...
class RobokassaRecurrent extends Robokassa {
	...
	private function getConnection(){
		try {
			...
			if($this->Debug) $this->debug('Подключение не удалось: ' . $e->getMessage());
		    echo 'Подключение не удалось: ' . $e->getMessage();
		}
	}
	function doRecurrentRedirect( $sum = 100, $desc = '', $invid = '0', $params = array(), $IncCurrLabel = 'ru', $recurrent = 1  ) {
		$recurrent = (int) $recurrent;
		if($recurrent > 0) $IncCurrLabel='BANKOCEAN3R';
		...
		if($recurrent > 0) $redirect .= '&Recurring=1';
		$this->updateRedirectField($invid, $redirect);
		if($this->Debug) $this->debug($invid, 'PAYMENT_RELEASE_INVID');
		header("X-Redirect: Powered by neatek");
		header("Location: ".$redirect);
	}
	function insertPayment($sum=0.0, $email='', $recurrent=false, $redirect_url='', $desc = '', $params = array(), $date = '') {
	...
	function insertRecurrent($sum=0.0, $prev_invid = 0, $invid= 0) {
		if(empty($date)) $date = date("Y-m-d H:i:s");
		$conn = $this->getConnection();
	...
	function setPaymentSuccess($invid=0) {
		if($this->Debug) $this->debug($invid,'SET_PAYMENT_SUCCESS');
		$conn = $this->getConnection();
	...
	function updateRedirectField($invid = 0, $redirect_url='') {
		$conn = $this->getConnection();
	...
	function getLatestForRecurrent() {
		$conn = $this->getConnection();
		...
		if($this->Debug) $this->debug($sql,'SQL_GET_RECURRENTS');
		$stmt = $conn->query($sql);
		$payments = array();
		while ($row = $stmt->fetch()) {
			if($this->Debug) $this->debug(print_r($row,true),'DO_RECURRENT');
			$payments[] = $row;
		}
		return $payments;
	}
	function updateLastRecurrent($invid=0) {
		$conn = $this->getConnection();
		...
		if($this->Debug) $this->debug(print_r($invid,true)."\r\n".date('Y-m-d H:i:s'), 'UPDATE_LAST_RECURRENT');
	}
	function getRecurrentPayment( $sum = 100, $desc = '', $invid = '0', $prev_invid = '0', $params = array(), $IncCurrLabel = 'ru'  ) {
	...
	function log_errors($data=array(),$name='') {
		file_put_contents('errors'.date('dmY').'.log',date('[H:i:s] ').$name."\r\nRESULT:::\r\n".print_r($data,true)."\r\n=====\r\n",FILE_APPEND);
	}
	function getRecurrent($invid = 0) {
		$conn = $this->getConnection();
		$invid = (int) $invid;
	...
	function cancelRecurrent($invid =0) {
		$invid = (int) $invid;
		$conn = $this->getConnection();
	...
	function getRecurrentPrevInvId($invid=0) { // get prev invid by autoInvid
		$conn = $this->getConnection();
		$invid = (int) $invid;
	...
	function doRecurrents() { // function for cron-job every 1min for 20 people
		...
		foreach ($recurrents as $key => $value) {
			...
			if($this->Debug) $this->debug(print_r($url,true), 'RECURRENT_URL');
			$result = file_get_contents($url);
			//$result = 'OK';
			if($this->Debug) $this->debug(print_r($result,true), 'RECURRENT_ANSWER');
